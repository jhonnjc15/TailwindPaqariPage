...
const ContactButton = () => {
    ...
    const PhoneNumber:number = 940780152
    const defaultMessage:string = "Saludos%0D%0AMe interesan sus productos, me gustaría ponerme en contacto con ustedes."
    const whatsappMessage:string = `https://api.whatsapp.com/send/?phone=51${PhoneNumber}&text=${defaultMessage}&type=phone_number&app_absent=0`
    const telegramUsarname = "VRaulJC"
    const telegramMessage:string = `https://telegram.me/${telegramUsarname}?text=${defaultMessage}`
    ...
    return (
        <div className='group fixed z-90 bottom-10 right-8 w-16 h-16 hover:h-48'>
            <div className='hidden group-hover:flex group-hover:flex-col h-32 justify-center items-center gap-4'>
                <a href={whatsappMessage} target="_blank" className='group-hover:animate-[comein_0.3s_ease-in-out_forwards] hover:drop-shadow-2xl'>
                    <svg
