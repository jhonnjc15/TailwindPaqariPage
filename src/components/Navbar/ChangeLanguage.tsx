...
const ChangeLanguage = ({ openLanguage, setOpenLanguage}:Props) => {
  ...
  return (
    ...
    <div  className={`group order-last bg-gray-200  rounded ${openLanguage ? 'rounded-br-none' : ''} flex relative ${NavbarStyles.WidthButton} h-[30px] sm:order-last ${NavbarStyles.SmWidthButton}`}>
      <button ref={anchorRef} className='w-1/3 h-[30px] sm:px-1'>
          <svg onClick={() => setOpenLanguage(!openLanguage) } xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="text-black transform transition duration-500 sm:group-hover:scale-125 group-hover:scale-110">
